...
// =========================================================
// Mise à jour du départ client (Ziguinchor / Carabane)
// =========================================================
if (isset($_POST['update_depart_client'], $_POST['billet_id'], $_POST['depart_client'])) {

    $departs_valides = ['Ziguinchor', 'Carabane'];
    $nouveau_depart = $_POST['depart_client'];

    if (in_array($nouveau_depart, $departs_valides, true)) {
        ...
        $stmt_update = $pdo->prepare("
            ...
            WHERE id = ?
        ");
        $stmt_update->execute([$nouveau_depart, $_POST['billet_id']]);
    }

    // Redirection (PRG) pour éviter la re-soumission et rester sur la même traversée
    $traversee_id_redirect = $_POST['traversee_id'] ?? '';
    header("Location: billets_send1.php?traversee_id=" . urlencode($traversee_id_redirect));
    exit;
}
        ...
        <?= htmlspecialchars($b['numero_place']) ?>
                        ...
                        <td>
                            ...
                            <form method="POST" class="d-flex align-items-center gap-1">
                                ...
                                <input type="hidden" name="billet_id" value="<?= $b['id'] ?>">
                                <input type="hidden" name="traversee_id" value="<?= htmlspecialchars($_GET['traversee_id'] ?? '') ?>">

                                <select name="depart_client" class="form-select form-select-sm" style="width:auto;">

                                    <option value="Ziguinchor" <?= (($b['depart_client'] ?? 'Ziguinchor') === 'Ziguinchor') ? 'selected' : '' ?>>
                                        Ziguinchor
                                    </option>

                                    <option value="Carabane" <?= (($b['depart_client'] ?? '') === 'Carabane') ? 'selected' : '' ?>>
                                        Carabane
                                    </option>
