...
if(isset($_GET['export']) && $_GET['export'] == 'csv'){
    ...
    $navireLabel = $bateau != '' ? $bateau : 'Tous les navires';

    $ligneSep   = str_repeat('=', 70);
    $ligneSepFine = str_repeat('-', 70);

    fwrite($output, $ligneSep . "\n");
    fputcsv($output, ['OSAMA — RAPPORT FINANCIER BILLETTERIE ET FRET'], ';');
    fwrite($output, $ligneSep . "\n");
    fwrite($output, "\n");

    fputcsv($output, ['Agence', 'Gare Maritime de Dakar'], ';');
    fputcsv($output, ['Période', 'du ' . date('d/m/Y', strtotime($debut)) . ' au ' . date('d/m/Y', strtotime($fin))], ';');
    fputcsv($output, ['Navire', $navireLabel], ';');
    fputcsv($output, ['Édité le', date('d/m/Y à H:i:s')], ';');

    fwrite($output, "\n");
    fwrite($output, $ligneSepFine . "\n");
    fputcsv($output, ['DÉTAIL DES VENTES — BILLETTERIE'], ';');
    fwrite($output, $ligneSepFine . "\n");
    fwrite($output, "\n");
    ...
    fputcsv($output, ['Document généré automatiquement — OSAMA © ' . date('Y')], ';');
...
// Libellé navire utilisé dans l'en-tête imprimable
$navireLabel = $bateau != '' ? $bateau : 'Tous les navires';
...
/* =========================================
   🖨️ ZONE IMPRESSION PDF (style template OSAMA)
========================================= */
...
<div class="container-fluid mt-4 px-2 px-md-4 no-print">
...
<!-- =========================================
     🖨️ VUE IMPRIMABLE (visible uniquement à l'impression / PDF)
     Reprend le template OSAMA fourni
========================================== -->
<div id="printReport">
    ...
    <div class="print-header">
        ...
        <div class="print-logo">
            <img src="../assets/logo-cosama.png" alt="COSAMA" class="logo-img">
        </div>

        <div class="print-title">
            RAPPORT FINANCIER<br>ÉTAT DES VENTES — BATOBI
        </div>
    ...
    <table class="print-info-table">
        ...
        <tr>
            <td>Navire</td>
            <td><?= htmlspecialchars($navireLabel) ?></td>
        </tr>
    ...
    <table class="print-table">
        ...
        <thead>
            ...
            <tr>
                ...
                <th>Navire</th>
                <th>Total billets</th>
                <th>Frais service (FCFA)</th>
                <th>Taxe portuaire (FCFA)</th>
        ...
        <?php if(count($ventes) > 0): ?>
            ...
            <tr>
                ...
                <td><?= htmlspecialchars($v['navire']) ?></td>
                <td><?= $v['total_billets'] ?></td>
                <td><?= number_format($v['total_frais'], 0, ',', ' ') ?></td>
                <td><?= number_format($v['taxe_portuaire'], 0, ',', ' ') ?></td>
                <td><?= number_format($v['total_montant'], 0, ',', ' ') ?></td>
            </tr>
        ...
        <tfoot>
            ...
            <tr>
                ...
                <td></td>
                <td><?= $totalBillets ?></td>
                <td><?= number_format($totalFrais, 0, ',', ' ') ?></td>
    ...
    <div class="print-footer">
        Document généré automatiquement — COSAMA © <?= date('Y') ?>
    </div>
