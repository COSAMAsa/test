...
/* =========================
   ...
   Complétez "DIAMBOGNE" avec le même format que
   "A.S.D" / "AGUENE" ci-dessous dès que vous aurez la liste.
   ⚙️ Ce bloc est synchronisé avec add_traversee.php.
========================= */
...
$plans_par_bateau = [
    ...
    "A.S.D" => [
        ...
        "Cabine 8 places Femme" => [
            ...
            "302.20A", "302.20B", "302.20C", "302.20D", "302.20E", "302.20F", "302.20G", "302.20H","302.03D",
        ],
...
// 🔒 Places automatiques à masquer du formulaire (affichage uniquement), par bateau
$places_auto_affichage_par_bateau = [
    "A.S.D" => [
   'Cabine 8 places Homme' => [
    '302.11A','302.11B','302.11C','302.11D',
    '302.11E','302.11F','302.11G','302.11H',
...
/* =========================
   BATEAU ACTUEL DU VOYAGE
   (utilisé pour afficher le bon plan par défaut)
========================= */

$bateauActuel = $traversee['bateau'] ?? '';

// Sécurité : si le bateau en base n'est pas dans la liste connue, on retombe sur le premier
if(!in_array($bateauActuel, $bateaux_disponibles)){
    $bateauActuel = $bateaux_disponibles[0];
}

function recalculerPlacesExistantes($places){
    $placesExistantes = [];
    foreach($places as $p){
        $placesExistantes[] = $p['numero_place'];
    }
    return $placesExistantes;
}

$placesExistantes = recalculerPlacesExistantes($places);
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    try {
        ...
        $bateauSelectionne = $_POST['bateau'];

        if(!in_array($bateauSelectionne, $bateaux_disponibles)){
        ...
        /* ==========================
           ...
           🔒 Les places automatiques/obligatoires du bateau
           sélectionné sont fusionnées ici (elles sont masquées
           du formulaire, donc jamais cochées manuellement).
        ========================== */

        $siegesChoisis = $_POST['sieges'] ?? [];

        $sieges_obligatoires = $sieges_obligatoires_par_bateau[$bateauSelectionne] ?? [];

        foreach($sieges_obligatoires as $type_place => $liste_obligatoire){

            if(!isset($siegesChoisis[$type_place])){
                $siegesChoisis[$type_place] = [];
            }

            foreach($liste_obligatoire as $numero){

                if(!in_array($numero, $siegesChoisis[$type_place])){
                    $siegesChoisis[$type_place][] = $numero;
                }
            }
        }
        ...
        /*
         * 2. Ajouter les nouvelles places cochées (+ places automatiques)
         */
        ...
        // 🔄 Rafraîchir les données affichées après enregistrement
        $stmt = $pdo->prepare("SELECT * FROM traversees WHERE id=?");
        $stmt->execute([$id]);
        $traversee = $stmt->fetch();
        ...
        $bateauActuel = $traversee['bateau'] ?? $bateauActuel;
        if(!in_array($bateauActuel, $bateaux_disponibles)){
            $bateauActuel = $bateaux_disponibles[0];
        }
...
<select class="form-select mb-3" name="depart" id="depart" required>
<option value="">Choisir départ</option>
<?php foreach(["DAKAR","ZIGUINCHOR","CARABANE"] as $d): ?>
<option value="<?= $d ?>" <?= $traversee['depart'] === $d ? 'selected' : '' ?>><?= $d ?></option>
<?php endforeach; ?>
</select>

<select class="form-select mb-3" name="destination" id="destination" required>
<option value="">Choisir destination</option>
<?php foreach(["DAKAR","ZIGUINCHOR","CARABANE"] as $d): ?>
<option value="<?= $d ?>" <?= $traversee['destination'] === $d ? 'selected' : '' ?>><?= $d ?></option>
<?php endforeach; ?>
</select>
...
<option value="<?= $b ?>" <?= $bateauActuel === $b ? 'selected' : '' ?>><?= $b ?></option>
<?php endforeach; ?>
</select>
...
<div class="col-md-4">
<label class="form-label small">➕ Type de la nouvelle place (optionnel)</label>
<input type="text" class="form-control" name="nouveau_type" placeholder="Ex: Pullman">
</div>
<div class="col-md-4">
<label class="form-label small">➕ Numéro de la nouvelle place (optionnel)</label>
<input type="text" class="form-control" name="nouvelle_place" placeholder="Ex: A2.20">
</div>
</div>

<button class="btn btn-success mt-2">💾 Enregistrer</button>
<a href="liste_traversees.php" class="btn btn-secondary mt-2">⬅️ Retour</a>

<h5 class="mb-3 mt-4">🚢 Plan d'embarquement</h5>

<?php foreach($bateaux_disponibles as $bateau_nom): ?>

<div class="plan-bateau <?= $bateau_nom === $bateauActuel ? 'actif' : '' ?>" data-bateau="<?= $bateau_nom ?>">

<h6 class="mb-3">Plan — <?= $bateau_nom ?></h6>

<?php
$planBateau = $plans_par_bateau[$bateau_nom] ?? [];
$places_auto_affichage = $places_auto_affichage_par_bateau[$bateau_nom] ?? [];
?>

<?php if(empty($planBateau)): ?>

<div class="alert alert-warning">
🚧 Le plan de sièges pour <strong><?= $bateau_nom ?></strong> n'est pas encore configuré dans le code.
</div>

<?php else: ?>

<?php foreach($planBateau as $type => $liste): ?>
...
if(
    ...
    &&
    in_array($numero, $places_auto_affichage[$type])
){
    continue;
}
?>
