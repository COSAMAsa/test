...
if(isset($_POST['form_action']) && $_POST['form_action'] === 'add_enfant'){
    ...
    if($billet_id > 0 && $nom_enfant !== '' && $age_valeur >= 0){
        ...
        if($age_ok){
            ...
            $enfant_message = [
                "type" => "success",
                "text" => "👶 Enfant « " . htmlspecialchars($nom_enfant, ENT_QUOTES, 'UTF-8') . " » ajouté avec succès."
            ];
...
elseif(isset($_POST['code'])){
    ...
    elseif($billet){
        ...
        ========================================= */
            ...
            // 📝 Traçabilité : on enregistre l'agent qui vient de valider ce billet
            ...
            $pdo->prepare("
                INSERT INTO embarquements (billet_id, agent, created_at)
                VALUES (?, ?, NOW())
...
function nom_complet($data){
    ...
    $nom    = trim($data['nom'] ?? '');
    return htmlspecialchars(trim($prenom . ' ' . $nom), ENT_QUOTES, 'UTF-8');
}

/* =========================================
   🧩 FONCTION UTILITAIRE : ÉCHAPPEMENT COURT
========================================= */
function e($value){
...
<div class="container mt-4">
    ...
    <?php if($result['type']=="success"): ?>
        ...
        🚢
        <?= e($result['data']['depart']) ?>
        →
        <?= e($result['data']['destination']) ?>
        ...
        💺 Place :
        <?= e($result['data']['type_place']) ?>

        <br>

        👤
        <?= e($result['data']['type_passager']) ?>
        -
        <?= e($result['data']['type_client']) ?>
    ...
    <?php if($result['type']=="multiple"): ?>
        ...
        <p class="text-muted" style="font-size:14px;">
            Sélectionnez le bon passager pour poursuivre l'embarquement.
        </p>
        ...
        <?php foreach($result['data'] as $b): ?>
            ...
            <div class="border rounded p-2 mb-2 bg-white text-start d-flex justify-content-between align-items-center flex-wrap gap-2">
                ...
                <div>
                    ...
                    🚢
                    <?= htmlspecialchars($b['depart'], ENT_QUOTES, 'UTF-8') ?>
                    →
                    <?= htmlspecialchars($b['destination'], ENT_QUOTES, 'UTF-8') ?>

                </div>

                <!-- 🔓 Sélection directe : soumet le code exact de CE billet pour
                     poursuivre le traitement (embarquement, déjà-embarqué, etc.)
                     au lieu de bloquer l'agent sur une simple liste. -->
                <form method="POST" class="m-0">
                    <input type="hidden" name="code" value="<?= htmlspecialchars($b['code_qr'], ENT_QUOTES, 'UTF-8') ?>">
        ...
        <div class="enfant-box shadow-sm">
            ...
            <?php if($enfants_list): ?>
                ...
                <ul class="list-group mb-3">
                    ...
                    <?php foreach($enfants_list as $e_item): ?>

                        <li class="list-group-item d-flex justify-content-between align-items-center">

                            <?= htmlspecialchars($e_item['nom_enfant'], ENT_QUOTES, 'UTF-8') ?>

                            <span class="badge bg-info text-dark">
                                <?= (int) $e_item['age_valeur'] ?>
                                <?= $e_item['age_unite'] == 'mois' ? 'mois' : 'an(s)' ?>
                            </span>
