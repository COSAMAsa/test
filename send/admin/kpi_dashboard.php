...
/* =========================================
   🚢 TAUX DE VENTE (billets émis / places disponibles)
   -> Basé sur TOUTES les traversées (passées + futures),
      cohérent avec les autres KPI du dashboard
   -> billets émis = statut 'valide' OU 'embarque'
      (un billet embarqué est un billet qui était valide)
   -> places disponibles = capacité totale (p.total)
   -> Calcul basé directement sur la table billets pour éviter
      toute désynchronisation avec le champ places.restant
========================================= */

// Capacité totale de toutes les traversées
$stmt = $pdo->query("
    SELECT SUM(p.total) as capacite_totale
    FROM traversees t
    JOIN places p ON t.id = p.traversee_id
");
$capacite_totale = (int)$stmt->fetchColumn();

// Billets émis (statut = valide OU embarque) sur toutes les traversées
$stmt = $pdo->query("
    SELECT COUNT(*)
    FROM billets b
    JOIN places p ON b.id_place = p.id_place
    JOIN traversees t ON t.id = p.traversee_id
    WHERE b.statut IN ('valide','embarque')
");
$billets_emis = (int)$stmt->fetchColumn();

if($capacite_totale > 0){
    $taux = round(($billets_emis / $capacite_totale) * 100, 2);
    // Garde-fou : ne jamais dépasser 100%
    $taux = min($taux, 100);
} else {
    $taux = 0;
}
