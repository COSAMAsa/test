...
AND LOWER(TRIM(b.type_passager)) NOT LIKE 'enfant%'
GROUP BY t.id
...
if (!empty($_GET['traversee_id'])) {
    ...
    if ($traversee) {
        ...
        // (comparaison insensible à la casse / espaces superflus)
        ...
        try {
            ...
            // On essaie d'abord avec le filtre Carabane (si applicable)
            ...
            // ⚠️ Si la colonne depart_client n'existe pas (ou autre souci SQL),
            // on retombe sur la requête sans filtre pour ne pas planter la page.
            error_log("manifeste_voyage.php - erreur SQL avec filtre carabane: " . $e->getMessage());
...
<div class="container-fluid mt-4 px-2 px-md-4">
 ...
 <a
        href="manifeste_enf.php?traversee_id=<?= $traversee_id ?>"
        target="_blank"
        class="btn"
        style="background:#6f42c1; color:#fff;"
        >
