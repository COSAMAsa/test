...
// 🔒 CORRECTIF BUG #2 (front-end) — sexe du passager transmis par acheter.php,
// pour griser/bloquer dès la sélection les cabines incompatibles.
$sexe = strtoupper($_GET['sexe'] ?? '');
$sexe = in_array($sexe, ['M', 'F']) ? $sexe : '';
...
/* =========================
   COMPATIBILITE GENRE / CABINE
========================= */

// 🔒 CORRECTIF BUG #2 (front-end) — une cabine "Homme" n'est proposable qu'à un
// passager 'M', une cabine "Femme" qu'à un passager 'F'. Mixte/Pullman : toujours ok.
// Cette vérification est en plus (pas à la place) du blocage déjà en place côté
// serveur dans acheter.php, qui reste la garantie ultime.
function placeCompatibleAvecSexe($type, $sexe){

    if($sexe === ''){
        // Sexe pas encore choisi : on n'empêche pas l'affichage, la sélection
        // sera de toute façon revalidée côté serveur au moment de l'achat.
        return true;
    }
...
/* 🔒 CORRECTIF BUG #2 (front-end) — cabine non compatible avec le sexe déclaré */
.incompatible{
...
<div class="legend">
    ...
    <div class="badge">
        ...
        Non disponible pour votre sexe
...
<?php
    $dispo = $s['restant'] > 0;
    $compatible = placeCompatibleAvecSexe($type, $sexe);
    $cliquable = $dispo && $compatible;
?>

<div

class="siege <?= !$dispo ? 'occupe' : (!$compatible ? 'incompatible' : '') ?>"

style="
background:
<?= !$dispo
    ? '#dc3545'
    : (!$compatible ? '#adb5bd' : couleurCabine($type))
?>;
"
...
<?php elseif($dispo && !$compatible): ?>

onclick="alert('🔒 Cette place (<?= htmlspecialchars($type, ENT_QUOTES) ?>) n\'est pas disponible pour le sexe déclaré du passager.')"
...
<?php if($dispo && !$compatible): ?>
<div class="restant-msg">Réservé <?= strpos(strtolower($type), 'homme') !== false ? 'aux hommes' : 'aux femmes' ?></div>
<?php endif; ?>
